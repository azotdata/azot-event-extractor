...
BAD_DOMAINS = ['amazon', 'doubleclick', 'twitter', 'facebook', 'clicannonces']
...
def valid_url(url, verbose=False, test=False):
    ...
    if url is None or len(url) < 11:
        if verbose:
            ...
            logging.debug('\t%s rejected because len of url is less than 11' % url)
        return False
    ...
    if r1 or r2:
        if verbose:
            ...
            logging.debug('\t%s rejected because len of url structure' % url)
        return False
    ...
    if len(path_chunks) > 0:
        ...
        if file_type and file_type not in ALLOWED_TYPES:
            if verbose:
                ...
                logging.debug('\t%s rejected due to bad filetype' % url)
            return False
    ...
    for GOOD in GOOD_PATHS:
        if GOOD.lower() in [p.lower() for p in path_chunks]:
            if verbose:
                ...
                logging.debug('%s verified for good path' % url)
            return True
    ...
    if len(path_chunks) <= 1:
        if tld == 'zinfos974':
            ...
            if verbose:
                ...
                logging.debug('%s caught for path chunks too small' % url)
            return False
    ...
    for b in BAD_CHUNKS:
        if b in path_chunks or b == subd:
            if verbose:
                ...
                logging.debug('%s caught for bad chunks' % url)
            return False
        ...
        if dash_count >= underscore_count:
            if tld not in [ x.lower() for x in url_slug.split('-') ]:
                if verbose:
                    ...
                    logging.debug('%s verified for being a slug' % url)
                return True
    ...
    if match_date is not None:
        if verbose:
            ...
            logging.debug('%s verified for date' % url)
        return True

    if verbose: 
        print '%s caught for default false' % url
        logging.debug('%s caught for default false' % url)
    return False
