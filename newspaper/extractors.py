...
class ContentExtractor(object):
    ...
    def get_publishing_date(self, url, doc):
        ...
        3. Pubdate from specific tag like time
        """

        def parse_date_str(date_str):
            try:
                datetime_obj = date_parser(date_str,fuzzy_with_tokens=True)
                return datetime_obj
            except:
    ...
    def get_category_urls(self, source_url, doc):
        ...
        for p_url in page_urls:
            ...
            if domain:
                ...
                if not subdomain_contains and \
                    ...
                    if self.config.verbose:
                        print ('elim category url %s for mobile '
                               'subdomain' % p_url)
                        logging.debug('elim category url %s for mobile '
                               'subdomain' % p_url)
                    continue
                else:
                ...
                #**********it is for testing the get category**********
